      ...
      const req = https.request(options, (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          ...
          https.get(res.headers.location, (r2) => {
            ...
            r2.on('data', c => rb += c);
            r2.on('end', () => {
              console.log(`ð Sheets OK (${r2.statusCode}): ${rb.slice(0, 80)}`);
              resolve(rb);
            });
          }).on('error', (err) => {
            console.error('â Redirect error:', err.message);
            resolve(null);
          });
          return;
        }
